...
const triggerVariants = cva(
  [
    'flex w-full items-center justify-between gap-2 rounded-md border bg-[var(--color-surface-overlay)] text-foreground shadow-sm',
    'transition-[border-color,box-shadow,background-color] duration-[var(--duration-base)] ease-[var(--ease-default)]',
    'focus:outline-none focus:ring-2 focus:ring-primary/20 disabled:cursor-not-allowed disabled:opacity-60 motion-reduce:transition-none',
    '[&>span]:truncate'
  ],
  {
    variants: {
      ...
      state: {
        default: 'border-[var(--color-neutral-300)] focus:border-primary',
...
const contentVariants = cva(
  'z-50 max-h-72 min-w-[var(--radix-select-trigger-width)] overflow-hidden rounded-md border border-[var(--color-neutral-300)] bg-[var(--color-surface-overlay)] text-foreground shadow-lg'
);

const itemVariants = cva(
  [
    'relative flex cursor-default select-none items-center rounded-sm py-2 pl-8 pr-3 text-sm outline-none',
    'focus:bg-[var(--color-neutral-100)] data-[disabled]:pointer-events-none data-[disabled]:opacity-50'
  ]
);

const groupLabelVariants = cva('px-3 py-2 text-xs font-semibold text-foreground-muted');
const searchShellVariants = cva('flex items-center gap-2 border-b border-[var(--color-neutral-300)] px-3 py-2 text-foreground-muted');
