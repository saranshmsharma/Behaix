  ...
  ({ alt, className, fallbackIcon, initials, name, size = 'md', src, ...props }, ref) => {
    ...
    return (
      <span ref={ref} aria-label={accessibleName} className={cx(avatarVariants({ size }), className)} role="img" {...props}>
        {src ? <img alt="" className={avatarImageVariants()} src={src} /> : fallbackText ? fallbackText : fallbackIcon ?? <User aria-hidden="true" className="size-1/2" />}
      </span>
    );
  }
);
...
export const AvatarGroup = React.forwardRef<HTMLDivElement, AvatarGroupProps>(
  ({ avatars, className, max = 4, size = 'md', ...props }, ref) => {
    ...
    return (
      <div ref={ref} className={cx(avatarGroupVariants(), className)} {...props}>
        ...
        {overflow > 0 ? (
          <span aria-label={`${overflow} more avatars`} className={cx(avatarOverflowVariants({ size }), '-ml-2')}>
            +{overflow}
          </span>
        ) : null}
      </div>
...
export { avatarVariants, getInitials, avatarGroupVariants, avatarImageVariants, avatarOverflowVariants };
